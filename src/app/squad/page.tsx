  ...
  return (
    <Panel className="rise">
      ...
      <div className="grid gap-6 px-4 py-5 sm:grid-cols-2">
        <div className="space-y-3">
          ...
          <p className="text-[13px] leading-relaxed text-muted">
            Creates a squad and gives you a link to paste into Discord. Whoever clicks it joins.
          </p>
          <Button
            variant="primary"
            disabled={busy}
        ...
        <div className="space-y-3">
          ...
          <p className="text-[13px] leading-relaxed text-muted">
            Paste the link or type the code. Case and the usual look-alike characters do not
            matter.
          </p>
          <form
      ...
      <p className="border-t border-line px-4 py-3 text-[12px] leading-relaxed text-muted">
        Your squad sees your nickname, level, faction, the map you are loading into, and which
        tasks you have done or are holding. Nothing else — not your logs, not your account.
      </p>
    </Panel>
  );
}
      ...
      <div className="space-y-4 px-4 py-4">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <code className="data flex-1 border border-line-bright bg-ground-2 px-2 py-1.5 text-[12px] break-all text-bone-dim">
              {link}
            </code>
            <Button
              onClick={() => {
  ...
  if (members.length < 2) {
    return (
      <Panel className="rise" style={{ animationDelay: "60ms" }}>
        ...
        <EmptyNote>
          Send someone your invite link. Once two of you are in, this shows the tasks you are
          both holding.
        </EmptyNote>
      </Panel>
    );
          ...
          return (
            <li key={member.id} className="flex flex-wrap items-center gap-3 px-4 py-2.5">
              ...
              <span
                className={cx(
                  "text-[14px]",
                  member.id === identity?.id ? "text-amber" : "text-bone",
                )}
              >
                {member.name}
              ...
              {member.level ? (
                <span className="data text-[11px] text-muted">Lv{member.level}</span>
              ) : null}
              {map ? <Pill tone="amber">{map.name}</Pill> : null}
              {!member.online ? (
                <span className="data ml-auto text-[10px] text-muted">offline</span>
              ) : null}
            </li>
          );
        })}
