...
export function TraderLevels() {
  ...
  const traders = useMemo(
    () => (data?.traders ?? []).filter((trader) => trader.levels.length > 1),
    [data],
  );
    ...
    <Panel className="rise" style={{ animationDelay: "120ms" }}>
      ...
      <div className="space-y-3 px-4 py-4">
        <p className="text-[12px] leading-relaxed text-muted">
          Blank means we have not been told, and each page reads that the cautious way for
          its own question. The task list assumes the requirement is met and marks the task
          unverified, rather than hiding work you can do. The crafts calculator assumes level
          1, rather than pricing an ingredient at an offer you may not have.
        </p>
