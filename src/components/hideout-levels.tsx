...
export function HideoutLevels() {
    ...
    return (
        <Panel className='rise' style={{ animationDelay: '150ms' }}>
            <PanelHeader title='Hideout' meta={`${recorded} of ${economy.stations.length} recorded`} />
            <div className='space-y-3 px-4 py-4'>
                <p className='text-[12px] leading-relaxed text-muted'>
                    Blank means we have not been told, and the sell check assumes you might still need the items. Set 0 for a station you have not built.
                </p>
                ...
                <ul className='divide-y divide-line'>
                    {economy.stations.map((station) => {
                        ...
                        const options: Array<number | null> = [null, ...Array.from({ length: max + 1 }, (_, level) => level)];

                        return (
                            <li key={station.id} className='flex flex-wrap items-center justify-between gap-2 py-2'>
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={cx(
                                                    ...
                                                    active ? 'border-amber bg-amber/15 text-amber' : 'border-line-bright text-muted hover:text-bone-dim',
                                                )}
                                            >
                                                {option ?? '?'}
                                            </button>
