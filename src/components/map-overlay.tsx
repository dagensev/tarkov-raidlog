 ...
 * showing.
 */
function prepare(text: string, baseLayer: string, activeFloor: MapFloor | null): Prepared | null {
...
export function MapOverlay({
    ...
    const box = useMemo(() => (prepared ? fitBox(prepared.aspect, area) : { width: 0, height: 0 }), [prepared, area]);
    ...
    const chip = 'stencil cursor-pointer border px-2 py-1 text-[10px] transition-colors';
    ...
    return (
        <div
            role='dialog'
            aria-modal='true'
            aria-label={`Raid map of ${map.name}`}
        ...
        >
            <header className='shrink-0 border-b border-line bg-panel/70'>
                <div className='flex flex-wrap items-center justify-between gap-4 px-4 pt-3 pb-2'>
                    <h2 className='stencil text-[13px] text-amber glow-amber'>{map.name} — raid map</h2>
                    <div className='flex flex-wrap items-center gap-3'>
                        {calibration.floors.length > 0 ? (
                            <div className='flex items-center gap-1'>
                                <button type='button' onClick={() => onFloor(null)} className={cx(chip, floor === null ? chipOn : chipOff)}>
                                    Ground
                                </button>
                                {calibration.floors.map((option) => (
                                    <button
                                        ...
                                        className={cx(chip, floor?.svgLayer === option.svgLayer ? chipOn : chipOff)}
                                    >
                                        {option.name}
                                    </button>
                                ))}
                        ...
                        <button ref={closeRef} type='button' onClick={onClose} title='Close the map (Esc)' className={cx(chip, chipOff)}>
                            Close ✕
                        </button>
                    </div>
                </div>
                                ...
                                return (
                                    ...
                                    >
                                        <button
                                            ...
                                            onPointerLeave={() => setHoveredKey((current) => (current === pin.key ? null : current))}
                                            onClick={(event) => {
                                            ...
                                            className={cx(
                                                ...
                                                pin.kind === 'zone' ? 'size-[10px] border-amber bg-amber/60' : 'size-[7px] border-bone-dim bg-bone-dim/40',
