...
/**
 * Status of the log link, as a lamp plus a word. Silent while watching: a lamp that
 * is always lit stops being read, so the strip only speaks up when the link needs
 * attention.
 */
function LogLamp() {
...
/** Tasks and item names, shown only while they are still arriving or missing. */
function DataLamp() {
    ...
    if (!loading && tasks && items) return null;
    ...
    const state = loading
        ...
        : error && !tasks
          ...
          : tasks
            ? { tone: 'moss' as const, text: 'Names loading', live: true }
    ...
    return (
        <div className='flex flex-wrap items-center gap-x-6 gap-y-2'>
            ...
            <span className='flex items-center gap-2'>
                ...
                <span className={cx('data text-[11px]', inRaid ? 'text-amber glow-amber' : 'text-bone-dim')}>{currentMap?.name ?? '—'}</span>
            </span>
        </div>
    );
}
            ...
            <header className='rise border-b border-line pt-6 pb-0'>
                <div className='flex flex-wrap items-end justify-between gap-4'>
                    <Link href='/' className='group flex items-center gap-3'>
                         ...
                         */}
                        <Image src='/raidlog-badge.png' alt='' aria-hidden width={192} height={192} loading='eager' className='h-12 w-12 shrink-0' />
                        ...
                        return (
                            <Link
                                ...
                                className={cx(
                                    ...
                                    active ? 'border-amber text-amber' : 'border-transparent text-muted hover:text-bone-dim',
                                )}
                            >
                                {item.label}
                            </Link>
