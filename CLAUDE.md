...
**tarkov.dev's JSON API**, not its GraphQL endpoint, which is retired and now 422s. See the header comment in `src/lib/tarkovdev/endpoints.ts`: documents are normalized (ids, not nested objects), display names live in separate `<path>_<lang>` translation dictionaries, and data is split by game mode. The mode comes from the log's `Session mode:` line unless overridden in settings. The 8.7 MB maps document is stripped to per-map metadata plus its `extracts`, which is the one piece of published geometry the app keeps and is what the raid map draws exits from. `client.ts` fetches a small core bundle first and defers the 16.7 MB item catalogue, so the task list renders before the big download finishes. The catalogue carries the prices and ages out after an hour (`PRICES_TTL_MS`); everything else lasts a day. `refreshData` only fetches what is behind, and `startPriceRefresh` re-checks every five minutes while the tab is visible.

**The game's own configuration**, for the handful of facts nothing above publishes. tarkov.dev's extract entries carry a position, a faction and a toll and nothing else, so what an exit *asks* of you — a Red Rebel and a paracord, an empty backpack slot, the generator running — is vendored in `src/lib/maps/extract-requirements.ts` from SPT's mirror of the game's `locations/<map>/base.json` and `globals.json`, with the wording taken from the game's own English locale. Same trade as `calibration.ts` and `crafts/fuel.ts`: a small table of static facts against a third-party dependency on every page load. `live-api.test.ts` fails if any key in it stops matching a published extract, which is the only thing keeping it honest.
...
- `src/lib/maps/` — `calibration.ts` vendors tarkov.dev's per-map bounds and rotation; `project.ts` turns a game coordinate into a fraction across the SVG with no map library; `viewport.ts` holds the zoom, pan and clamp arithmetic so it can be tested without a DOM. `pins.ts` and `extracts.ts` flatten tasks and map exits into one entry per mark, `extract-requirements.ts` is the vendored table the latter joins on the raw extract name, and both flatteners take `foldedMapIds` — that is what keeps Night Factory's objectives *and* its exits on Factory. They fold differently, though: a task's zone names the map it is on, so `pins.ts` rewrites that reference, while an extract hangs off the map document itself, so `extracts.ts` merges one map's list into another's and dedupes on the translated name. Ids cannot do that job — the two Factory documents give the same gate different GUIDs — and the dedupe must not run *within* a map, because Factory publishes two separate doors both called "Gate 3".
...
- `src/lib/barters/` — the barters calculator, one row per barter, built on `crafts/routes.ts`, `crafts/pricing.ts` and `crafts/plan.ts`: those three are generic, and the crafts calculator was only the first caller. What differs is what a row means. A craft's product is found in raid and sells; a barter's is not, so it cannot be listed on the flea and pricing it the way a craft is priced makes almost every barter a loss — true, and useless. So `barter-row.ts` carries two figures: `savings`, what buying the item outright would have cost less what the trade costs, which is why anyone barters and what the table opens sorted on, and `resell`, the honest cash-out through `dispose(..., foundInRaid: false)`. The value side is `buyPrice` directly rather than the planner, because the planner's cheapest route to the product is the row's own barter and every saving would come out zero. `buyLimit` is the only per-refresh ration in the data, and gets a column of its own; `savingsPerRestock` multiplies the two and lives in the breakdown rather than the table.
- `src/lib/table/` — `window.ts`, the row windowing arithmetic, and `item-box.ts`, which sizes an item in a calculator's process cell to its stash footprint at a fixed pixels-per-cell, scaling down only what will not fit. Under `sell/` until the crafts table proved there was nothing about selling in it. `PROCESS_HEIGHT` in `components/calculators/process.tsx` derives from those limits, and both tables' `ROW_HEIGHT` derives from it — the windowing needs every collapsed row to be exactly that tall, so the three move together.
- `src/lib/tasks/` — filtering, sorting and map options for the task list. `members.ts` is the one filter that does not read your logs: it picks which squad members' published progress the status filter applies to, which is how a task you have never accepted can appear. An empty selection means no narrowing, the same rule the flea page's chip groups use.
...
- **Every cached document is versioned; not every one refuses to serve an old copy.** `SELL_INDEX_VERSION`, `ECONOMY_BUNDLE_VERSION` and `CORE_BUNDLE_VERSION` each make their `*Behind` predicate refetch. A cache lives up to a day, so `useSellIndex` and `useEconomy` *also* refuse to serve a copy of the wrong version — otherwise today's components read yesterday's fields until the download lands. `CoreBundle` deliberately has no such gate: it is the task list, and refusing it would blank the whole app for the length of a download. Its new fields are written defensively in `denormalize` instead (`raw.extracts ?? []`), so an old cache renders a map with no exits rather than crashing. Bump the version in the same change that adds or renames a field, either way.
